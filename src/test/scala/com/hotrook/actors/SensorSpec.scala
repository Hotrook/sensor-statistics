...
class SensorSpec() extends TestKit(ActorSystem("DirectoryScannerSpec"))
  ...
  "Sensor" should {
    ...
    "return None values if all measurements were invalid" in {
      ...
      supervisor.expectMsg(Sensor.SensorSummary(None, None, None, 3, 0))
    }
    ...
    "not process message if incorrect sensor id" in {
      ...
      supervisor.expectMsg(Sensor.SensorSummary(None, None, None, 0, 0))
    }
    ...
    "calculate correct min, max and avg" in {
      ...
      supervisor.expectMsg(Sensor.SensorSummary(Some(2), Some(1), Some(3), 3, 3))
    }
